...
class PaymentService {
    ...
    async createDonationPayment(data) {
        try {
            ...
            const donation = await Donation_1.Donation.findByPk(data.donationId);
            if (!donation) {
    ...
    async handlePostPaymentActions(payment) {
        try {
            ...
            else if (payment.type === Payment_1.PaymentType.DONATION && payment.donationId) {
                // Update donation status
                const donation = await Donation_1.Donation.findByPk(payment.donationId);
                if (donation) {
                    await donation.update({ status: 'completed' });
                    logger_1.default.info('Donation completed', { donationId: payment.donationId });
                }
            }
        }
        catch (error) {
    ...
    async getTopCustomers() {
        ...
        const allPayments = await this.repository.findAll({
            ...
            include: [{ model: User_1.User, as: 'user', attributes: ['id', 'email'] }],
        });
