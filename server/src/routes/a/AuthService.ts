...
export class AuthService {
   ...
   */
  async signupAdvertiser(data: SignUpRequestDto): Promise<SignUpResponseDto> {
    const {
      username,
      email,
      password,
   ...
   */
  async createSportsAdmin(data: SignUpRequestDto): Promise<User> {
    try {
  ...
  async createAdmin(data: SignUpRequestDto): Promise<SignUpResponseDto> {
    try {
      logger.info('Admin sign up started', { email: data.email })
