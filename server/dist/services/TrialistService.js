...
class TrialistService {
    ...
    async createTrialist(data) {
        ...
        if (existingTrialist) {
            throw new errors_1.AppError('A trialist with this email already exists', 409);
        }
        // Validate age (must be at least 14 years old)
        const age = this.calculateAge(new Date(data.dob));
        if (age < 14) {
            throw new errors_1.AppError('Trialist must be at least 14 years old', 400);
        }
    ...
    async getTrialistById(id) {
        ...
        if (!trialist) {
            throw new errors_1.AppError('Trialist not found', 404);
        }
        return trialist;
    }
    ...
    async uploadFiles(videoFile, cvFile) {
        const result = {};
