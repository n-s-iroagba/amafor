...
class LineupService {
    ...
    async createLineup(data) {
        try {
            ...
            if (!fixture) {
                throw new errors_1.AppError('Fixture not found', 404);
            }
            ...
            if (playerStatus !== 'ACTIVE') {
                throw new errors_1.AppError(`Player is not eligible to play (status: ${playerStatus})`, 400);
            }
            ...
            if (existingLineup) {
                throw new errors_1.AppError('Player is already in the lineup for this fixture', 400);
            }
            ...
            if (playerPrimaryPosition && !this.isValidPositionTransition(playerPrimaryPosition, data.position)) {
                logger_1.default.warn(`Player ${player.name} is playing out of position: ${data.position} (primary: ${playerPrimaryPosition})`);
            }
    ...
    async getLineupById(id) {
        ...
        if (!lineup) {
            throw new errors_1.AppError('Lineup entry not found', 404);
        }
        return lineup.toJSON();
    }
    ...
    async updateLineup(id, data) {
        try {
            ...
            if (!updatedLineup) {
                throw new errors_1.AppError('Failed to retrieve updated lineup', 500);
            }
    ...
    async setFixtureCaptain(fixtureId, playerId) {
        try {
            ...
            if (!playerLineup) {
                throw new errors_1.AppError('Player is not in the lineup for this fixture', 400);
            }
            const success = await this.repository.setCaptain(fixtureId, playerId);
            if (!success) {
                throw new errors_1.AppError('Failed to set captain', 500);
            }
    ...
    async bulkCreateFixtureLineup(fixtureId, lineups) {
        try {
            ...
            if (invalidPlayers.length > 0) {
                throw new errors_1.AppError(`Invalid player IDs: ${invalidPlayers.map(p => p.playerId).join(', ')}`, 400);
            }
            ...
            if (duplicatePlayers.length > 0) {
                throw new errors_1.AppError(`Duplicate players in lineup: ${duplicatePlayers.join(', ')}`, 400);
            }
    ...
    async replaceFixtureLineup(fixtureId, lineups) {
        try {
            ...
            if (lineups.length > 23) {
                throw new errors_1.AppError('Maximum squad size is 23 players', 400);
            }
