    ...
    async recalculateStandings(leagueId) {
        ...
        for (const stat of statistics) {
            ...
            if (stat.matchesPlayed > 0) {
                stat.avgGoalsPerFixture = Number((stat.goalsFor / stat.matchesPlayed).toFixed(2));
                stat.avgGoalsConcededPerFixture = Number((stat.goalsAgainst / stat.matchesPlayed).toFixed(2));
            }
            await stat.save();
        }
    }
            ...
            try {
                ...
                const aggregateResult = await this.model.findOne({
                    ...
                    attributes: [
                        ...
                        [sequelize_1.Sequelize.fn('SUM', sequelize_1.Sequelize.col('matchesPlayed')), 'totalFixturees'],
                    ],
                    raw: true,
                });
                ...
                const totalFixturees = aggregateResult?.totalFixturees || 0;
                ...
                const actualFixturees = Math.floor(totalFixturees / 2);
                const summary = {
                    totalGoals,
                    averageGoalsPerFixture: parseFloat(avgGoals.toFixed(2)),
                    totalFixturees: actualFixturees,
                    totalTeams,
                ...
                span.setAttributes({
                    ...
                    averageGoalsPerFixture: summary.averageGoalsPerFixture,
                    totalFixturees: summary.totalFixturees,
