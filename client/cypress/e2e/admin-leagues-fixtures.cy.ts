describe('Admin Leagues & Fixtures Management', () => {
    beforeEach(() => {
        ...
        // Leagues: useGet<League[]> → data used directly
        cy.intercept('GET', '**/api/leagues', {
            statusCode: 200,
            body: {
                success: true, data: [
                    { id: 1, name: 'Premier League', season: '2023/2024', friendly: false, createdAt: '2023-01-01T00:00:00Z' },
                    { id: 2, name: 'FA Cup', season: '2023/2024', friendly: true, createdAt: '2023-02-01T00:00:00Z' }
                ]
            }
        }).as('getLeaguesList');
        ...
        // Fixtures
        cy.intercept('GET', '**/api/fixtures/league/*', {
            statusCode: 200,
            body: {
                success: true, data: [
                    { id: 1, leagueId: 1, homeTeam: 'Arsenal', awayTeam: 'Chelsea', date: '2024-01-15T15:00:00Z', venue: 'Emirates Stadium', status: 'completed', homeScore: 2, awayScore: 1, createdAt: '2023-06-01T00:00:00Z' }
                ]
            }
        }).as('getFixturesByLeague');

        // Players for lineup selection
        cy.intercept('GET', '**/api/players', {
            statusCode: 200, body: {
                success: true, data: [
                    { id: 1, name: 'Bukayo Saka', position: 'Forward', jerseyNumber: 7, imageUrl: '' }
                ]
            }
        }).as('getPlayers');

        cy.session('admin-leagues-fixtures-session', () => {
    ...
    it('should create a new league', () => {
        ...
        cy.get('[data-testid="input-league-name"]', { timeout: 15000 }).type('Championship');
