...
export function useObservability() {
  ...
  const trackDonation = useCallback(
    (amount: number, transactionId: string, isRecurring = false) => {
      metrics.recordDonation(amount, transactionId, isRecurring);
      log("info", `Donation made: ₦${amount}`, {
        amount,
        transactionId,
        isRecurring,
      });
  ...
  const trackRegistration = useCallback(
    (type: BusinessMetric["data"]["registrationType"]) => {
  ...
  const trackPerformance = useCallback(
    (metricName: string, value: number, threshold?: number) => {
...
function getLastUserActions(userId: string, limit: number = 5): any[] {
  ...
  const userLogs = logs.filter(
    (log) => log.context.userId === userId || log.metadata?.userId === userId,
  );
