 ...
 * Compose New Article
 * 
 * Editorial interface for drafting and publishing new articles with media enrichment and SEO metadata.
 * 
 * @screen SC-027
 * @implements REQ-CMS-01
 * @usecase UC-CMS-01 (Manage Articles)
 * @requires SRS-I-043 (Articles API - POST /admin/articles)
 * @performance NFR-PERF-01
 * @observability SRS-OBS-041 Monitor publishing success rates and media upload latency
 */
"use client";
...
const CustomEditor = dynamic(
  () => import("@/features/articles/components/Editor"),
  {
    ssr: false,
  },
);
...
export default function NewArticlePage() {
  ...
  const { post, isPending: loading } = usePost(API_ROUTES.ARTICLES.CREATE);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/dashboard/admin/cms/articles/${res.data.id}`);
    } catch (err: any) {
      console.error(err);
      const errorMessage =
