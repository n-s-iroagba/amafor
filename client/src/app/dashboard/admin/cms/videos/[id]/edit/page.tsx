...
export default function EditVideo() {
  ...
  const {
    ...
    loading: isLoading,
    error: fetchError,
  } = useGet<{ data: Video }>(API_ROUTES.VIDEOS.VIEW(id as string));
  ...
  const handleVideoSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validTypes.includes(file.type)) {
      setErrors((prev) => ({
        ...prev,
        video: "Please select a valid video file (MP4, WebM, OGG, MOV)",
      }));
      return;
    }
    ...
    if (file.size > maxSize) {
      setErrors((prev) => ({
        ...prev,
        video: "Video size must be less than 500MB",
      }));
      return;
    }
  ...
  const handleThumbnailSelect = (
    ...
    if (!validTypes.includes(file.type)) {
      setErrors((prev) => ({
        ...prev,
        thumbnail: "Please select a valid image file (JPEG, PNG, WebP, GIF)",
      }));
      return;
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            >
              ...
              >
                <h3 className="text-sm font-semibold text-sky-800 mb-3">
                  Video Information
                </h3>
