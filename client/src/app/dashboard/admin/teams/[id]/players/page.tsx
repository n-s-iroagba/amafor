                ...
                <div className="container mx-auto max-w-7xl px-4 py-8">
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sky-300 text-sm">Each player belongs to exactly one team at any given time (BR-TM-03). Transfers log history (BR-TM-04/09).</p>
                        </div>
                    ...
                    <div className={`${showAddPanel ? 'lg:col-span-2' : 'lg:col-span-3'}`}>
                        <div className="bg-white rounded-xl shadow-sm border border-sky-200 overflow-hidden">
                            <div className="px-6 py-4 border-b border-sky-200">
                                <h2 className="font-bold text-sky-800">Current Squad ({team?.players.length ?? 0})</h2>
                            </div>
                            {!team?.players.length ? (
                                <div className="p-12 text-center"><Users className="w-12 h-12 text-sky-200 mx-auto mb-4" /><p className="text-sky-500">No players assigned.</p></div>
                            ) : (
                                <div className="divide-y divide-sky-100">
                                    {(team?.players ?? []).map(player => (
                                        <div key={player.id} className="px-6 py-4 flex items-center justify-between hover:bg-sky-50">
                                            <div className="flex items-center gap-4">
                                                <span className="w-8 h-8 rounded-full bg-sky-100 flex items-center justify-center text-xs font-bold text-sky-600">{player.squadNumber}</span>
                                                <div>
                                                    ...
                                                    <span className="text-xs text-sky-500">{player.position}</span>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <button onClick={() => handleSetCaptain(player.id)} title={player.isCaptain ? 'Current captain' : 'Set as captain'} className={`p-2 rounded-lg transition-colors ${player.isCaptain ? 'text-amber-500 bg-amber-50' : 'text-sky-400 hover:text-amber-500 hover:bg-amber-50'}`}>
                                                    <Star className="w-4 h-4" />
                                                </button>
                                                <Link href={`/dashboard/admin/players/${player.id}/edit`} title="Transfer player" className="p-2 rounded-lg text-sky-400 hover:text-sky-600 hover:bg-sky-50 transition-colors">
                                                    <ArrowRightLeft className="w-4 h-4" />
                                                </Link>
                                                <button onClick={() => handleRemove(player.id)} title="Remove from team" className="p-2 rounded-lg text-sky-400 hover:text-red-600 hover:bg-red-50 transition-colors">
                                                    <X className="w-4 h-4" />
                                                </button>
                                            </div>
                                        </div>
                    ...
                    {showAddPanel && (
                        <div className="bg-white rounded-xl shadow-sm border border-sky-200 overflow-hidden">
                            <div className="px-6 py-4 border-b border-sky-200 flex items-center justify-between">
                                <h2 className="font-bold text-sky-800">Unassigned Players</h2>
                                <button onClick={() => setShowAddPanel(false)} className="text-sky-400 hover:text-sky-600"><X className="w-4 h-4" /></button>
                            </div>
                            <div className="p-4">
                                <div className="relative mb-3">
                                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-sky-400" />
                                    <input type="text" placeholder="Search players..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-sky-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-200" />
                                </div>
                            </div>
                            <div className="divide-y divide-sky-100 max-h-[400px] overflow-y-auto">
                                {unassigned.length === 0 ? (
                                    <div className="p-6 text-center"><p className="text-sm text-sky-500">No unassigned players found.</p></div>
                                ) : unassigned.map(player => (
                                    <div key={player.id} className="px-6 py-3 flex items-center justify-between hover:bg-sky-50">
                                        <div>
                                            <p className="font-medium text-sky-800 text-sm">{player.name}</p>
                                            <p className="text-xs text-sky-500">{player.position}</p>
                                        </div>
                                        <button onClick={() => handleAssign(player.id)} disabled={assigning} className="px-3 py-1.5 bg-sky-800 text-white rounded-lg text-xs font-medium hover:bg-sky-700 transition-colors flex items-center gap-1">
