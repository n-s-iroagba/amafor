        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
            <div className="bg-gradient-to-r from-slate-800 to-slate-900 text-white">
                <div className="container mx-auto max-w-7xl px-4 py-8">
                    <Link href="/dashboard/admin/teams" className="inline-flex items-center text-slate-400 hover:text-white text-xs font-bold uppercase tracking-widest mb-6 transition-colors">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-3 mt-1">
                                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${team.isActive ? 'bg-green-500/20 text-green-300' : 'bg-slate-500/20 text-slate-300'}`}>
                                        {team.isActive ? 'Active' : 'Inactive'}
                                    </span>
                                    <span className="text-slate-400 text-sm">{team.players.length} players</span>
                                    {captain && <span className="text-slate-400 text-sm flex items-center gap-1"><Star className="w-3 h-3 text-amber-400" /> {captain.name}</span>}
                                </div>
                            </div>
                        </div>
            ...
            <div className="container mx-auto max-w-7xl px-4 py-8">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
                        <h2 className="font-bold text-slate-800">Squad ({team.players.length})</h2>
                    ...
                    {team.players.length === 0 ? (
                        <div className="p-12 text-center">
                            ...
                            <p className="text-slate-500 mb-4">No players assigned yet.</p>
                            <Link href={`/dashboard/admin/teams/${id}/players`} className="px-6 py-2 bg-slate-800 text-white rounded-lg text-sm hover:bg-slate-700 transition-colors">
                                Assign Players
                            </Link>
                        </div>
                    ) : (
                                ...
                                <div key={player.id} className="px-6 py-4 flex items-center justify-between hover:bg-slate-50">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <span className="text-sm text-slate-500">{player.position}</span>
                                        </div>
                                    </div>
                                    <Link href={`/dashboard/admin/players/${player.id}`} className="text-xs text-slate-400 hover:text-slate-600 transition-colors">View Profile</Link>
                                </div>
                            ))}
                        </div>
                    )}
