...
export default function EditRssFeedSource() {
  ...
  useEffect(() => {
    const fetchRssFeed = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await api.get(API_ROUTES.FEEDS.VIEW(id as string));
        
        if (response && response.data) {
          const feedData: RssFeedSource = response.data;
          setName(feedData.name);
          setFeedUrl(feedData.feedUrl);
          setCategory(feedData.category);
        } else {
          setErrors({
            general: 'Failed to load RSS feed data. Please try again.'
          });
        }
      } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.put(API_ROUTES.FEEDS.MUTATE(id as string), {
        name: name.trim(),
      ...
      setErrors({
        general: error.response?.data?.message || 'Failed to update RSS feed source. Please try again.',
      });
    } finally {
...
const getCategoryInfo = (cat: RssFeedSourceCategory) => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-sky-50 to-sky-100 py-8 flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="w-8 h-8 animate-spin text-sky-600 mx-auto mb-4" />
          <p className="text-gray-600">Loading RSS feed data...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold">
                  Edit RSS Feed Source
                </h1>
                <p className="text-sky-100 text-sm sm:text-base mt-1">
                  Update the RSS feed source details
                </p>
              </div>
            </div>
          </div>
