                ...
                <div className="container mx-auto max-w-7xl px-4 py-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div>
                            ...
                            <p className="text-slate-300 text-sm">Schedule trial days, log training attendance, and track match sessions (BR-ADV-03, BR-ADV-04).</p>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <div className="flex items-center justify-between mb-6">
                            <button onClick={() => handleNav(-1)} className="p-2 hover:bg-slate-100 rounded-lg transition-colors"><ChevronLeft className="w-5 h-5 text-slate-600" /></button>
                            <h2 className="text-xl font-bold text-slate-800">{monthName} {year}</h2>
                            <button onClick={() => handleNav(1)} className="p-2 hover:bg-slate-100 rounded-lg transition-colors"><ChevronRight className="w-5 h-5 text-slate-600" /></button>
                        </div>
                      ...
                      ${isSelected ? 'bg-slate-800 text-white' : isToday ? 'bg-sky-100 text-sky-700' : 'hover:bg-slate-100 text-slate-700'}
                    `}
                                    >
                                        {day}
                    ...
                    <div>
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 h-full">
                            ...
                            {loading ? (
                                ...
                                <div className="p-8 text-center"><CalendarDays className="w-10 h-10 text-slate-200 mx-auto mb-3" /><p className="text-sm text-slate-500">Click a date to see sessions.</p></div>
                            ) : selectedSessions.length === 0 ? (
                                <div className="p-8 text-center"><CalendarDays className="w-10 h-10 text-slate-200 mx-auto mb-3" /><p className="text-sm text-slate-500">No sessions on this day.</p></div>
                            ) : selectedSessions.map(session => (
                                <div key={session.id} className="p-6 border-b border-slate-100 last:border-0">
                                        ...
                                        {session.attendees.map(att => (
                                            <div key={att.id} className="flex items-center justify-between text-sm">
                                                ...
                                                <div className="flex items-center gap-1">
                                                    <button onClick={() => handleAttendance(session.id, att.id, 'present')} className={`p-1 rounded ${att.status === 'present' ? 'text-green-600' : 'text-slate-300 hover:text-green-500'} transition-colors`}>
                                                        <CheckCircle className="w-4 h-4" />
                                                    </button>
                                                    <button onClick={() => handleAttendance(session.id, att.id, 'absent')} className={`p-1 rounded ${att.status === 'absent' ? 'text-red-500' : 'text-slate-300 hover:text-red-400'} transition-colors`}>
                                                        <XCircle className="w-4 h-4" />
                                                    </button>
                                                    {att.status === 'pending' && <Clock className="w-4 h-4 text-slate-300" />}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
