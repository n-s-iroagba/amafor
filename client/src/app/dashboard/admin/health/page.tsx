...
export default function InfrastructureHealthPage() {
  ...
  const [lastCheck, setLastCheck] = useState('2m ago');
  ...
  const components = [
    { name: 'Core API Services', status: 'Healthy', latency: '42ms', load: '14%', icon: <Server className="w-4 h-4" /> },
    { name: 'PostgreSQL Database', status: 'Healthy', latency: '8ms', load: '32%', icon: <Database className="w-4 h-4" /> },
    { name: 'Media Storage (CDN)', status: 'Optimal', latency: '124ms', load: '5%', icon: <Globe className="w-4 h-4" /> },
    { name: 'Auth Node (ISO Stack)', status: 'Healthy', latency: '12ms', load: '11%', icon: <Lock className="w-4 h-4" /> },
  ];

  const integrations = [
    { name: 'Paystack Settlement', status: 'Connected', lastSync: '2m ago', icon: <Database className="w-3.5 h-3.5" /> },
    { name: 'Google Analytics 4', status: 'Syncing', lastSync: '14m ago', icon: <Globe className="w-3.5 h-3.5" /> },
    { name: 'Email Service', status: 'Connected', lastSync: '45m ago', icon: <Mail className="w-3.5 h-3.5" /> },
    { name: 'Video Hosting', status: 'Connected', lastSync: '12m ago', icon: <Play className="w-3.5 h-3.5" /> },
  ];
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-[#2F4F4F] p-10 rounded-[3rem] text-white overflow-hidden relative shadow-2xl">
               <div className="flex items-center justify-between mb-12">
                  ...
                  <span className="text-[9px] font-bold text-gray-400 uppercase font-mono">Live Stream: 240,102 RPS</span>
               </div>
               <div className="h-48 flex items-end justify-between space-x-2">
                  {[40, 60, 45, 80, 55, 70, 40, 50, 65, 30, 90, 40, 55, 70, 35, 60].map((h, i) => (
